...
| 🔍 틀린그림찾기 | `/spot` | 매번 새로 그려지는 그림 · 혼자 기록 · 같은 그림에서 먼저 찾기 1:1 대전 |
...
`/spot`. 그림 파일이 없습니다 — **seed 하나로 장면을 SVG 로 생성**하고(공원 · 바다 · 밤거리 · 우주), 그중 N 개 물체를 색 · 크기 · 위치 · 좌우반전 · 회전 · 개수 · 삭제로 바꿔 짝을 만듭니다. 그래서 어디서든 같은 seed 면 같은 그림이고, 저작권 걱정도 없습니다.

- **난이도** — 쉬움 5곳 / 보통 7곳 / 어려움 10곳. 배경은 무작위 또는 지정.
- **혼자** — 시간 기록. 오답 +5초, 힌트 +20초(힌트를 쓰면 기록은 남지 않음). 난이도별 최고 기록은 `localStorage`. 그림 번호(`#seed`) 칩을 누르면 같은 그림 링크(`/spot?seed=…&d=…`)가 복사됩니다.
...
규칙은 `shared/spot.js`(장면 생성 · 차이 · 판정 · SVG)와 `shared/spotversus.js`(대전)에 있고, 화면 코드(`public/js/spot.js`)는 부르고 그리기만 합니다. 대전 화면의 관전/교대/채팅 부품은 `public/js/vsui.js` 로 지뢰찾기와 공유합니다.
...
shared/         서버와 브라우저가 함께 쓰는 순수 로직 (의존성 없음)
  ...
  spot.js         틀린그림찾기 — seed 로 장면 생성, 차이 만들기, 판정, SVG 그리기
...
| `{t:'spot', x, y}` | (틀린그림) 그림 찍기 (장면 좌표 480×360). 본인에게 `{t:'spot_result', hit, index, lockedUntil}` 이 먼저 온다 |
...
- `test/spot.test.js` — 틀린그림 생성 (seed 결정성, 차이 수·종류, 판정, SVG, 정답 은닉)
