...
| 🔎 숨은그림찾기 | `/hidden` | 그림 속 숨은 물건 찾기 · 쉬움/보통/어려움/매우 어려움/극한 · 그림 골라서 기록 도전 |
...
`/hidden`. 그림 파일(`public/hidden/<난이도>/NN.webp`)과 정답 위치(`public/hidden/puzzles.json`)로 돌아갑니다. 난이도 탭 → 썸네일 갤러리에서 그림을 골라 시작합니다.

- **규칙** — 그림 속 물건을 찾아 누르면 초록 원 + 목록 아이콘에 체크. 같은 물건이 여러 곳에 있으면 아무거나 하나. 오답 +5초, 힌트(3초간 위치 표시) +20초. 그림 밖(제목·목록 영역)을 눌러도 오답이 아닙니다.
- **기록** — 그림마다 최고 기록을 `localStorage` 에 저장. 갤러리 카드에 ✓와 기록이 붙고, 상단 칩에 난이도별 완료 수가 보입니다. `/hidden?p=easy-03` 처럼 링크로 바로 열 수 있습니다.
- **화면** — 그림 전체가 한 화면에 들어오게 맞추고, 🔍 확대 버튼으로 2배 확대 + 스크롤(모바일은 핀치 줌도 됨).
- **난이도 5단계** — 쉬움 10장 · 보통 10장 · 어려움 10장이 들어 있고, 매우 어려움 · 극한은 그림이 들어오면 탭이 켜집니다.

### 그림 추가하기

1. 그림을 `public/hidden/<difficulty>/NN.webp` 에 넣고, `node scripts/thumbs.mjs` 로 썸네일(`public/hidden/thumbs/<id>.jpg`)을 만듭니다. *(Playwright 필요)*
2. `puzzles.json` 에 항목을 추가합니다. `items` 는 비워 두어도 됩니다.
3. 브라우저에서 `/hidden?p=<id>&edit=1` 을 열면 **정답 편집기**가 켜집니다 — 물건을 고르고 그림을 누르면 정답 원이 생기고, 원을 다시 누르면 지워집니다. "JSON 복사"로 결과를 `puzzles.json` 에 붙여 넣으면 끝.

```jsonc
{ "id": "easy-01", "difficulty": "easy", "title": "숲속 피크닉",
  "image": "/hidden/easy/01.webp", "thumb": "/hidden/thumbs/easy-01.jpg",
  "width": 1055, "height": 1491,
  "scene": [10, 155, 795, 1470],                  // 누를 수 있는 영역 — 바깥은 무시
  "list": { "x": 895, "y0": 270, "y1": 1345 },    // 그림 안 목록 아이콘 위치 (또는 {x, ys:[…]} / {pts:[{x,y},…]})
  "items": [ { "name": "버섯", "targets": [ { "x": 65, "y": 945, "r": 45 } ] }, … ] }
```
...
| `{t:'watch', game, code, name}` | (지뢰찾기) 관전 입장 |
...
- `test/hidden.test.js` — 숨은그림찾기 (puzzles.json 검증 · 정답 판정 · 오답/또 찍음/장면 밖 · 힌트 · 기록)
